...
LRMultiClass <- function(X, y, Xt, yt, numIter = 50, eta = 0.1, lambda = 1, beta_init = NULL){
  ...
  for(k in 1:length(Y_list)){ # go through the beta obj and if Y is equal to the class indicator is 1
  ...
  for(k in 1:numIter){
    ...
    #print(error_test)
    objective[k+1] <-   (-sum(ind_train * log(pk)) + (lambda/2) * sum(beta_init^2))
  }
